...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20000,
        help="Number of CodeSearchNet examples to use (0 = all samples, default: 20000).",
    )
    parser.add_argument(
        "--beam-width",
        type=int,
...
def main() -> int:
    ...
    dataset = preprocessor.download_dataset(
        language=args.language, max_samples=max_samples
    )
    if not dataset:
    ...
    metadata = {
        ...
        "max_samples": args.max_samples if max_samples is not None else "all",
        "beam_width": args.beam_width,
        "target_length": args.target_length,
        "n_gram": args.n_gram,
